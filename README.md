...
- Windows は Win32 API 直書き、macOS は AppKit 直書き。どちらも NativeAOT で、GUI フレームワーク (WPF / WinForms) を使いません。
...
- **操作のカスタマイズ** — 130 以上のコマンドをキーとマウス (クリック / ダブルクリック / 長押し / ホイール / 押しながらホイール / X ボタン / マウスジェスチャー) に自由に割り当て
- **表示言語** — 日本語 / 英語の切り替え
- **効果音** — ページ・本の移動が端で止まったときに .wav を再生 (既定はオフ)
- **その他** — 多重起動の抑止 (設定で許可も可)、ウインドウ位置と全画面状態の復元、エクスプローラ / Finder で開く、ファイルの削除 (ゴミ箱行き / 直接削除)、設定ファイルの再読み込み
